...
export class SummaryComponent implements OnInit {
  ...
  onDeleteClick(row: Department) {
    ...
    conformationPopup({
      title: 'Are you sure?',
      text: 'You wont be delete this  application department',
      type: 'warning',
      ...
      if (dismiss.value === true) {
        ...
        .subscribe((response: IResponse) => {
          if (!response.hasError) {
            console.log(row.id);
            console.log(response.error);
            this.getAllDepartmentsApplication();
          } else {
            console.log(row.id);
            console.log(response.error);
          }
        ...
        conformationPopup(
          'Deleted!',
        'Your application department has been deleted.',
        'success'
        );
        ...
        conformationPopup(
          'Cancelled',
          'Your application department is safe :)',
          'error'
        );
      }
  ...
  private getAllDepartmentsApplication() {
    this.departmentService
      .getAllDepartment()
      .subscribe((response: IResponse) => {
        if (!response.hasError) {
