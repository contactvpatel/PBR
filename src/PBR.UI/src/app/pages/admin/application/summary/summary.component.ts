...
export class SummaryComponent implements OnInit {
  ...
  onDeleteClick(row: Applications) {
    ...
    conformationPopup({
      title: 'Are you sure?',
      text: 'You wont be delete this application account',
      type: 'warning',
      ...
      if (dismiss.value === true) {
        ...
        .subscribe((response: IResponse) => {
          if (!response.hasError) {
            ...
            this.getAllApplicationAccounts();
          } else {
           console.log(response.error);
          }
        });
        conformationPopup(
          'Deleted!',
        'Your application account has been deleted.',
        'success'
        );
        ...
        conformationPopup(
          'Cancelled',
          'Your application account is safe :)',
          'error'
        );
      }
  ...
  private getAllApplicationAccounts() {
    this.applicationsService
      .getAllApplications()
      .subscribe((response: IResponse) => {
        if (!response.hasError) {
